$(document).ready(function () {
    $('#nome_Prefeitura').keypress(function (e) {
        if (e.which === 13) {
            BuscaConsulta();
        }
    });
...
function BuscaConsulta() {
    ...
    let nome_Prefeitura = $('#nome_Prefeitura').val();
    ...
    table = $("#dataTable").DataTable({
        ...
        "ajax": {
            ...
            "url": ResolveUrl('Prefeitura/ListaPaginada'),
            "type": "Post",
            "datatype": "json",
            "data": {
        ...
        "columns": [
            { "data": "nome_Prefeitura", orderable: false, "name": "nome_Prefeitura" },
            { "data": "descricao", orderable: false, "name": "descricao" },
            { "data": "endereco", orderable: false, "name": "endereco" },
            {
                "data": "cgc", orderable: false, "name": "cgc", render: function (valor) {
            ...
            },
            {
                "data": null, "orderable": false, width: '100px', render: function (data) {
                    ...
                    let html = '<span id="btn-status-' + data.id + '"><button type="button" data-toggle="tooltip" data-placement="left" onclick="window.location.href=\'/prefeitura/formulario/' + data.id + '\'" title="Editar" class="btn btn-info btn-sm btn-table" style="padding:.10rem .5rem"> ' +
...
function excluir(elemento) {
    ...
    PrefeituraExcluir(id).then(function () {
        BuscaConsulta();
        notification('Prefeitura removido com sucesso', 'success');
        unloading(elemento);
...
function ativar(id) {
    PrefeituraAtivar(id).then(function () {
        BuscaConsulta();
        notification('Prefeitura ativado', 'success');
    }, function (err) {
        notification('Erro ao ativar a prefeitura', 'danger');
        console.error(err);
    });
}

function desativar(id) {
    PrefeituraDesativar(id).then(function () {
        BuscaConsulta();
        notification('Prefeitura desativado', 'success');
    }, function (err) {
        notification('Erro ao desativar a prefeitura', 'danger');
        console.error(err);
    });
}
...
function Imprimir(elemento) {   
    ...
    loading(elemento);
    ImprimirPrefeituras(nome_Prefeitura, descricao, endereco, cgc, status).then(function (obj) {
        ...
        unloading(elemento);
        notification('Erro ao imprimir lista de prefeituras', 'danger');
    });
}

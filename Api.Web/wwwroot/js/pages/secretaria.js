...
function BuscaConsulta() {
    ...
    table = $("#dataTable").DataTable({
        ...
        "ajax": {
            ...
            "url": ResolveUrl('Secretaria/ListaPaginada'),
            "type": "Post",
            "datatype": "json",
            "data": {
        ...
        "columns": [
            ...
            { "data": "responsavel", orderable: false, "name": "responsavel" },
            {
                "data": "orgao", orderable: false, "name": "orgao", render: function (value) {
                    return value.orgaos
                }
            },
            {
            ...
            
            {
                "data": null, "orderable": false, width: '100px', render: function (data) {
                    ...
                    let html = '<span id="btn-status-' + data.id + '"><button type="button" data-toggle="tooltip" data-placement="left" onclick="window.location.href=\'/secretaria/formulario/' + data.id + '\'" title="Editar" class="btn btn-info btn-sm btn-table" style="padding:.10rem .5rem"> ' +
...
//        if (data.count === 0) {
//        } else {
//            document.getElementById("orgaoId").add(new Option('Todos', null));
...
function excluir(elemento) {
    ...
    SecretariaExcluir(id).then(function () {
        BuscaConsulta();
        notification('Secretaria removido com sucesso', 'success');
        unloading(elemento);
...
function ativar(id) {
    SecretariaAtivar(id).then(function () {
        BuscaConsulta();
        notification('Secretaria ativado', 'success');
    }, function (err) {
        notification('Erro ao ativar a secretaria', 'danger');
        console.error(err);
    });
}

function desativar(id) {
    SecretariaDesativar(id).then(function () {
        BuscaConsulta();
        notification('Secretaria desativado', 'success');
    }, function (err) {
        notification('Erro ao desativar a secretaria', 'danger');
        console.error(err);
    });
}
function Imprimir(elemento) {
    ...
    loading(elemento);
    ImprimirSecretarias(descricao, responsavel, orgaoId, status).then(function (obj) {
        ...
        unloading(elemento);
        notification('Erro ao imprimir lista de secretaria', 'danger');
    });
}
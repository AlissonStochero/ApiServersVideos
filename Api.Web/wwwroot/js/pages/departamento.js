...
function BuscaConsulta() {
    ...
    table = $("#dataTable").DataTable({
        ...
        "ajax": {
            ...
            "url": ResolveUrl('Departamento/ListaPaginada'),
            "type": "Post",
            "datatype": "json",
            "data": {
                descricao: descricao,
        ...
        "columns": [
            ...
            },
            {
                "data": null, "orderable": false, width: '100px', render: function (data) {
                    ...
                    let html = '<span id="btn-status-' + data.id + '"><button type="button" data-toggle="tooltip" data-placement="left" onclick="window.location.href=\'/departamento/formulario/' + data.id + '\'" title="Editar" class="btn btn-info btn-sm btn-table" style="padding:.10rem .5rem"> ' +
...
function excluir(elemento) {
    ...
    DepartamentoExcluir(id).then(function () {
        BuscaConsulta();
        notification('Departamento removido com sucesso', 'success');
        unloading(elemento);
...
function ativar(id) {
    DepartamentoAtivar(id).then(function () {
        BuscaConsulta();
        notification('Departamento ativado', 'success');
    }, function (err) {
        notification('Erro ao ativar o departamento', 'danger');
        console.error(err);
    });
}

function desativar(id) {
    DepartamentoDesativar(id).then(function () {
        BuscaConsulta();
        notification('Departamento desativado', 'success');
    }, function (err) {
        notification('Erro ao desativar o departamento', 'danger');
        console.error(err);
    });
}
...
function Imprimir(elemento) {
    ...
    loading(elemento);
    ImprimirDepartamentos(descricao, status).then(function (obj) {
        ...
        unloading(elemento);
        notification('Erro ao imprimir lista de departamentos', 'danger');
    });
}